...
namespace MNP.Managers
{
    public class TcpManager : MonoBehaviour
    {
        ...
        public int InitNewServer(string ip = "127.0.0.1", int port = 6132)
        {
            ...
            {
                Debug.LogError("VNPTcp管理器：注册端口已被使用的Tcp服务器");
            }
            int id = counter;
            counter++;
            ...
            Debug.Log("VNPTcp管理器：注册新的Tcp服务器，ID#" + id);
            return id;
        }
        ...
        public void StartServer(int id)
        {
            ...
            {
                Debug.LogError("VNPTcp管理器：启动未知的Tcp服务器");
                return;
            }
        ...
        public void StopServer(int id)
        {
            if (!servers.ContainsKey(id))
            {
                Debug.LogError("VNPTcp管理器：停止未知的Tcp服务器");
                return;
            }
            TcpServer server = servers[id];
            server.Stop();
        ...
        public void ReleaseServer(int id)
        {
            if (!servers.ContainsKey(id))
            {
                Debug.LogError("VNPTcp管理器：释放未经注册的Tcp服务器");
                return;
            }
            TcpServer server = servers[id];
            if (server.IsRunning)
            {
                Debug.LogError("VNPTcp管理器：关闭运行中的Tcp服务器#" + id);
                return;
            }
            servers.Remove(id);
